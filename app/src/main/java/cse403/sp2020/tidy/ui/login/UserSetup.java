...
public class UserSetup extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mJoinHouseholdButton.setOnClickListener(v -> {
            ...
            if (str_link.startsWith("https://tidy.household/")) {
                ...
                model.setCurrentUser(mAuth.getUid(), u -> {
                    if (u != null) {
                        ...
                        model.setCurrentHousehold(householdId, h -> {
                            // Household was created successfully
                            if (h != null) {
                                model.updateCurrentUser(u, nu -> {
                                    if (nu == null) {
                                ...
                                CharSequence text = "Unable to join shared household - double check the link";
        ...
        mCreateHouseholdButton.setOnClickListener(v -> {
            ModelInterface model = new ModelInterface(mFirestore);
            model.setCurrentUser(mAuth.getUid(), u -> {
                if (u != null) {
                    String displayName = mAuth.getCurrentUser().getDisplayName();
                    ...
                    model.createHousehold(new HouseholdModel(), h -> {
                        // Household was created successfully
                        if (h != null) {
                                model.updateCurrentUser(u, nu -> {
                                    if (nu == null) {
